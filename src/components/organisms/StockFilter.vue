    ...
    <div class="content">
      ...
      <div class="item">
        ...
        <div v-for="item in checkboxItems" :key="item.label" class="mb10">
          <vs-checkbox v-model="item.checked" color="#1F74FF">
            {{ item.label }}
          </vs-checkbox>
        </div>
      </div>
...
import { StockFilterParams } from '~/pages/owned_items_list/index'
  ...
  setup(
    ...
    const filterParams = computed(
      (): StockFilterParams => ({
        dateFrom: from.value,
        dateTo: to.value,
        includePublic: state.checkboxItems[0].checked,
        includePrivate: state.checkboxItems[1].checked,
        includeLending: state.checkboxItems[2].checked,
